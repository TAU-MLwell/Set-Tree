...
class LossFunction(metaclass=ABCMeta):
    ...
    def update_terminal_regions(self, tree, X_set, y, residual, raw_predictions, sample_weight, sample_mask,
        ...
        terminal_regions = tree.apply(X_set)
        ...
        for leaf_sn in np.unique(terminal_regions):
            self._update_terminal_region(tree, masked_terminal_regions,
                                         leaf_sn, X_set, y, residual,
                                         raw_predictions[:, k], sample_weight)

        # update predictions (both in-bag and out-of-bag)
        leafs_values = np.array([tree.leafs[i].value for i in range(len(tree.leafs))])
        raw_predictions[:, k] += \
            learning_rate * leafs_values.take(terminal_regions, axis=0)
...
class LeastSquaresError(RegressionLossFunction):
    ...
    def update_terminal_regions(self, tree, X_set, y, residual, raw_predictions,
                                sample_weight, sample_mask, learning_rate=0.1, k=0):
        # update predictions
        raw_predictions[:, k] += learning_rate * tree.predict(X_set).ravel()

    def _update_terminal_region(self, tree, terminal_regions, leaf, X_set, y,
...
class LeastAbsoluteError(RegressionLossFunction):
    ...
    def _update_terminal_region(self, tree, terminal_regions,
                                leaf, X_set, y, residual, raw_predictions, sample_weight):
...
class BinomialDeviance(ClassificationLossFunction):
    ...
    def _update_terminal_region(self, tree, terminal_regions,
        ...
        """
        locs = np.where(terminal_regions == leaf)[0]
        residual = residual.take(locs, axis=0)
        y = y.take(locs, axis=0)

        sample_weight = sample_weight.take(locs, axis=0) if sample_weight is not None is not None else np.ones(len(locs))
        ...
        if abs(denominator) < 1e-150:
            tree.leafs[leaf].value = 0.0
        else:
            tree.leafs[leaf].value = numerator / denominator
...
class ExponentialLoss(ClassificationLossFunction):
    ...
    def _update_terminal_region(self, tree, terminal_regions,
        ...
        locs = np.where(terminal_regions == leaf)[0]
        raw_predictions = raw_predictions.take(locs, axis=0)
        y = X_set.y.take(locs, axis=0)
        sample_weight = sample_weight.take(locs, axis=0) if sample_weight is not None is not None else np.ones(len(locs))
        y_ = 2. * y - 1.
